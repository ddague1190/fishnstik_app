...
const SubcategoryList = ({ category, justPath }) => {
        ...
        <>
          ...
          <div className='subcategory-list__container'>
            <span
              ...
              className='subcategory-list--panleft'>
              <i className='fas fa-chevron-left'></i>
            </span>
            <div className='subcategory-list__window'>
              <div className='subcategory-list' ref={ref}>
                {Object.values(subcategories[category]).map(
                  (subcategory, index) =>
                    subcategory.path ? (
                      <Link
                        key={index}
                        to={`/products/${category}/${subcategory.path}`}
                        className='subcategory-item'>
                        <img
                          className='subcategory-item__image'
                          src={subcategory["image"]}
                          alt={`image_of_${subcategory["name"]}`}
                        />
            ...
            <span
              onClick={() => setMove(move + 1)}
              className='subcategory-list--panright'>
              <i className='fas fa-chevron-right'></i>
            </span>
          </div>

          <h3 className='heading--secondary'>
            All {category === "all" ? "products" : category}
          </h3>
        </>
      )}
    </div>
